...
{$mode DelphiUnicode} // Dit staat ook in de project opties, maar het lijkt er op dat Codetools daar niet kijkt

interface

uses
  LCLType, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, ExtCtrls,
  StdCtrls, ComCtrls, Grids, Menus, Generics.Collections,
...
begin
  ...
  begin
    ...
    if aParams.GeselecteerdeShape then
      ...
      { dit werkt bij mij niet goed in Lazarus. Iemand anders had er ook last van
      al was dit in aggpas, zie https://forum.lazarus.freepascal.org/index.php?topic=23336.0
...
begin
  ...
  begin
    ...
    myCanvas.FillRect(0, 0, pbLiniaal.Width, 26);
  end
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        RightToLeft := False;
        EndEllipsis := False;
        myCanvas.TextRect(R, aParams.X-5, 10, IntToStr(myMoment), myTextStyle);
      end;
    end;
  end;
end;
...
procedure TframeMidiEditor.FrameMouseWheel(Sender: TObject; Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
begin
  { in fp is de MousePos relatief tov het frame }
  if (MousePos.X >= pPaintBox.Left + sbVerticaal.Left)
  and (MousePos.Y <= (sbVerticaal.Top + sbVerticaal.Height)) then
    FModel.ZoomVerticaal(WheelDelta > 0)
  else if (MousePos.Y >= pPaintBox.Top + sbHorizontaal.Top)
    and (MousePos.X <= (sbHorizontaal.Left + sbHorizontaal.Width)) then
